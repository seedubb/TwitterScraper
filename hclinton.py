...
f = open('hclinton_tweet_data.csv', 'w')
...
try:
    user_timeline = twitter.get_user_timeline(screen_name='@HillaryClinton',count=100)
...
while len(user_timeline) != 0: 
    try:
        user_timeline = twitter.get_user_timeline(screen_name='@HillaryClinton',count=100,max_id=user_timeline[len(user_timeline)-1]['id']-1)
    except TwythonError as e:
        print e
    #print len(user_timeline)
    for tweet in user_timeline:
